    ...
    func newMessage(_ message: Message) {
        ...
        if message.authorId != DriftDataStore.sharedInstance.auth?.enduser?.userId{
            if let index = checkSectionsForMessages(message){
                ...
                if let createdAt = sections[0].first?.createdAt, (Calendar.current as NSCalendar).component(.day, from: createdAt as Date) ==  (Calendar.current as NSCalendar).component(.day, from: Date()){
